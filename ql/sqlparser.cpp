...
public:
    ...
    RC Parse(ParsedQuery &out) {
        ...
        else if (match(TokenType::ALTER)==0) {
            if (match(TokenType::TABLE)==0) return parseAlterTable(out);
        }
...
//        if (match(TokenType::RPAREN)) return QL_CREATEINDEXERR;
    ...
    RC parseAlterTable(ParsedQuery &q) {
        q.type = SQLType::ALTER_TABLE;

        if (peek().type != TokenType::IDENTIFIER) return QL_ALTERTABLEERR;
            q.alterTableName = AllocCopyString(next().text);
         //if (match(TokenType::ADD))
        //return QL_ALTERTABLEERR;
        do {
            AttrInfo attr;
            // 解析列名
            if (peek().type != TokenType::IDENTIFIER) return QL_ALTERTABLEERR;
            ...
            if (typeStr == "INT") {
                ...
                if (match(TokenType::LPAREN)) return QL_ALTERTABLEERR;
                if (peek().type != TokenType::INT_LITERAL) return QL_ALTERTABLEERR;
                attr.attrLength = std::stoi(next().text);
                if (match(TokenType::RPAREN)) return QL_ALTERTABLEERR;
                attr.attrType = STRING;
            } else {
                return QL_ALTERTABLEERR; // 无效类型
            }

            q.attrList.push_back(attr); // 假设 ParsedQuery 的 attrList 用于存储新增列
        } while (!match(TokenType::COMMA));
